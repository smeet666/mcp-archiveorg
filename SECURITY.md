...
This server is a read-only client for the Internet Archive. It holds no credentials, needs no
API key, opens no port, and writes nothing back to the Internet Archive. That rules out most
of what a vulnerability report usually concerns.

What remains is worth reporting:

- **Anything that lets a caller reach a host other than the Internet Archive.** The URLs are
  built from a fixed base; an argument that escapes it is a real finding.
- **Anything upstream text can do to the caller.** Titles, descriptions and scanned passages
...
Rate limiting by the Internet Archive, or the Internet Archive being down, is the upstream's business and
