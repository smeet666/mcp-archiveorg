...
a passage attributed to a book that does not contain it is a real bug, and it is only visible to
...
It reads the Internet Archive and returns what it reads. It does not write anything back, it
...
- **Every result carries its source.** Content that came from the Internet Archive goes out
  with a link to the page it came from, and the text block keeps that link even
  when it has to be shortened.
- **The server paces itself.** The Internet Archive is a non-profit that charges nobody. The minimum
...
`IA_LIVE=1 npm run test:live`, which does: it makes one request per route
against https://archive.org and checks the fields the parsers depend on. Run it if you
...
The tools are `search_inside`, `search_items`, `get_item`, `get_snapshot`, `list_snapshots` and `search_books`. A new tool is a bigger conversation than a new field on
...
## When the Internet Archive changes
